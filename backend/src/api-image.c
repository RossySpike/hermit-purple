...
void *post_api_image(stream_cursor *cursor, server_machine *machine,
  ...
  unsigned long long content_length_val =
      strtoull(content_length + value_start, NULL, 10);

  printf("DEBUG: Content-Length value = %llu\n", content_length_val);
  SUPPORTED_FILETYPE ft = FT_UNKNOWN;
  unsigned long long my_idx = get_idx(); // Temporal: retorna 1
  ...
  snprintf(filename, BUFFER, "%s%llu.%s", IMG_ORIGINAL_DIR, my_idx,
           file_extension(ft));
  fd = open(filename, O_CREAT | O_WRONLY | O_APPEND, 0644);
  if (fd < 0) {
    snprintf(send_buffer, BUFFER, "HTTP/1.1 500 Internal Server Error\r\n\r\n");
    write(get_client_fd(local_machine), send_buffer, strlen(send_buffer));
    goto cleanup;
  }

  // Escribir los bytes del body que ya tenemos
  write(fd, read_buffer + body_offset, bytes_en_buffer);
